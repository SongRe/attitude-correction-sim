...
void RenderPipeline(PhysicsSimProperties *sim_properties, double timestep)
{
    ...
    const glm::mat4 view = glm::lookAt(
        glm::vec3(3.0f, 3.0f, 3.0f), // camera position -> usually (3.0f, 3.0f, 3.0f)
        glm::vec3(0.0f, 0.0f, 0.0f), // camera target position
        glm::vec3(0.0f, 1.0f, 0.0f)  // up vector in world space
    );
    const glm::mat4 projection = glm::perspective(glm::radians(55.0f), (float)WINDOW_WIDTH / (float)WINDOW_HEIGHT, 0.1f, 100.0f);
    // const glm::mat4 projection = glm::ortho(-3.0f, 3.0f, -3.0f, 3.0f, 0.1f, 100.0f);
    ...
    while (IsRunning(sim_properties) && !glfwWindowShouldClose(window))
    {
        ...
        // no translation
        glm::mat4 model_sat = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, 0.0f));

        // rotates according to the current rigidbody attitude
        const quat &attit = sim_properties->rbody.attit;
        model_sat = model_sat * glm::toMat4(glm::quat(attit.w, attit.v.x, attit.v.y, attit.v.z));
    ...
    // if the window was closed manually, terminate the other threads
    if (glfwWindowShouldClose(window))
    {
        sim_properties->running_mutex.lock();
        sim_properties->running = false;
        sim_properties->running_mutex.unlock();
    }
...
unsigned int GenerateProgram(const std::string vertex_path, const std::string fragment_path)
{
    ...
    const unsigned int shader_prog = glCreateProgram();
